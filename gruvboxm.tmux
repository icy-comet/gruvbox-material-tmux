...
main() {
  ...
  custom_path="$(get_tmux_option "@gruvboxm_custom_plugin_dir" "${PLUGIN_DIR}/custom")"
  ...
  # load local theme
  local theme
  theme="$(get_tmux_option "@gruvboxm_flavour" "dark")"
  # NOTE: Pulling in the selected theme by the theme that's being set as local
  # variables.
  # https://github.com/dylanaraps/pure-sh-bible#parsing-a-keyval-file
  # shellcheck source=./gruvboxm-frappe.tmuxtheme
  while IFS='=' read -r key val; do
    ...
    eval "local $key"="$val"
  done <"${PLUGIN_DIR}/gruvboxm-${theme}.tmuxtheme"

  # status general
  local status_default status_justify status_background message_background
  status_default=$(get_tmux_option "@gruvboxm_status_default" "on")
  # shellcheck disable=SC2121
  set status "$status_default"

  status_justify=$(get_tmux_option "@gruvboxm_status_justify" "left")
  set status-justify "$status_justify"

  status_background=$(get_tmux_option "@gruvboxm_status_background" "theme")
  ...
  pane_status_enable=$(get_tmux_option "@gruvboxm_pane_status_enabled" "no") # yes
  pane_border_status=$(get_tmux_option "@gruvboxm_pane_border_status" "off") # bottom
  pane_border_style=$(get_tmux_option "@gruvboxm_pane_border_style" "fg=${thm_gray}")
  pane_active_border_style=$(
    get_tmux_option "@gruvboxm_pane_active_border_style" \
      "#{?pane_in_mode,fg=${thm_yellow},#{?pane_synchronized,fg=${thm_magenta},fg=${thm_orange}}}"
  )
  pane_left_separator=$(get_tmux_option "@gruvboxm_pane_left_separator" "█")
  pane_middle_separator=$(get_tmux_option "@gruvboxm_pane_middle_separator" "█")
  pane_right_separator=$(get_tmux_option "@gruvboxm_pane_right_separator" "█")
  pane_number_position=$(get_tmux_option "@gruvboxm_pane_number_position" "left") # right, left
  ...
  window_status_separator=$(get_tmux_option "@gruvboxm_window_separator" "")
  setw window-status-separator "$window_status_separator"

  window_left_separator=$(get_tmux_option "@gruvboxm_window_left_separator" "█")
  window_right_separator=$(get_tmux_option "@gruvboxm_window_right_separator" "█")
  window_middle_separator=$(get_tmux_option "@gruvboxm_window_middle_separator" "█ ")
  window_number_position=$(get_tmux_option "@gruvboxm_window_number_position" "left") # right, left
  window_status_enable=$(get_tmux_option "@gruvboxm_window_status_enable" "no")       # right, left
  ...
  status_left_separator=$(get_tmux_option "@gruvboxm_status_left_separator" "")
  status_right_separator=$(get_tmux_option "@gruvboxm_status_right_separator" "█")
  status_connect_separator=$(get_tmux_option "@gruvboxm_status_connect_separator" "yes")
  status_fill=$(get_tmux_option "@gruvboxm_status_fill" "icon")

  status_modules_left=$(get_tmux_option "@gruvboxm_status_modules_left" "")
  ...
  status_modules_right=$(get_tmux_option "@gruvboxm_status_modules_right" "application session")
